# main.py
import streamlit as st
import pandas as pd
from datetime import datetime
import io
import zipfile
import json # <-- Import json module

# --- Import Core Components ---
from state_manager import initialize_state
from constants import ( # Use constants
    DATA_TYPES, USE_CASES, PRIVACY_METHODS,
    TABULAR_METHODS, EXCEL_AUG_METHODS, IMAGE_METHODS, TEXT_METHODS, GRAPH_METHODS
)
from utils import draw_graph, image_to_bytes, nx

# --- Import Page Rendering & Action Functions ---
from tabular_generator import render_tabular_page, generate_tabular_data_action
from excel_generator import render_excel_page, generate_excel_data_action
from image_generator import render_image_page, generate_image_data_action
from text_generator import render_text_page, generate_text_data_action
from graph_generator import render_graph_page, generate_graph_data_action

# --- Initialize Session State ---
initialize_state()

# --- Streamlit App Configuration ---
st.set_page_config(layout="wide", page_title="Synthetic Data Generator")
# --- MODIFICATION 3: Change App Heading ---
st.title("Synthetic Data Generator")
# --- END MODIFICATION 3 ---


# === Sidebar: Global Configuration ===
st.sidebar.header("1. Define Target Data")
selected_data_type = st.sidebar.selectbox( "Select Data Type", DATA_TYPES, index=DATA_TYPES.index(st.session_state.config.get('data_type', DATA_TYPES[0])), key="data_type_selector" )
if selected_data_type != st.session_state.config.get('data_type'):
    st.session_state.config['data_type'] = selected_data_type
    # Reset results when changing type
    st.session_state.results = {'data': None, 'message': None, 'error': None, 'is_generating': False}
    st.rerun()

st.session_state.config['use_case'] = st.sidebar.selectbox( "Intended Use Case", USE_CASES, index=USE_CASES.index(st.session_state.config.get('use_case', USE_CASES[0])) )
st.sidebar.header("2. Determine Privacy (Optional)")
st.session_state.config['privacy_level'] = st.sidebar.selectbox( "Privacy Method", PRIVACY_METHODS, index=PRIVACY_METHODS.index(st.session_state.config.get('privacy_level', PRIVACY_METHODS[0])) )
if st.session_state.config['privacy_level'] != "None": st.session_state.config['privacy_epsilon'] = st.sidebar.number_input( "Epsilon (ε)", 0.1, 10.0, st.session_state.config.get('privacy_epsilon', 1.0), 0.1 )

st.sidebar.header("3. Generation Method")
current_data_type = st.session_state.config['data_type']
available_methods = []
if current_data_type == "Tabular": available_methods = TABULAR_METHODS
elif current_data_type == "Excel Augmentation": available_methods = EXCEL_AUG_METHODS
elif current_data_type == "Image (Basic Shapes)": available_methods = IMAGE_METHODS
elif current_data_type == "Text (Basic)": available_methods = TEXT_METHODS
elif current_data_type == "Graph (Basic Random)": available_methods = GRAPH_METHODS

current_method = st.session_state.config.get('generation_method')
method_disabled = (current_data_type == "Excel Augmentation") # Excel Aug only has one method

# Auto-select method if needed or invalid
if not available_methods:
    st.session_state.config['generation_method'] = None
elif current_method not in available_methods:
    st.session_state.config['generation_method'] = available_methods[0]

# If still None (e.g., for Graph at start), select the first valid one
if st.session_state.config['generation_method'] is None and available_methods:
     st.session_state.config['generation_method'] = available_methods[0]

st.session_state.config['generation_method'] = st.sidebar.selectbox(
    "Select Method",
    available_methods,
    index=available_methods.index(st.session_state.config['generation_method']) if st.session_state.config['generation_method'] in available_methods else 0,
    disabled=method_disabled,
    key="gen_method_selector"
)


# === Main Area: Type-Specific Configuration & Generation ===
st.header(f"Configure: {current_data_type}")
if current_data_type == "Tabular": render_tabular_page()
elif current_data_type == "Excel Augmentation": render_excel_page()
elif current_data_type == "Image (Basic Shapes)": render_image_page()
elif current_data_type == "Text (Basic)": render_text_page()
elif current_data_type == "Graph (Basic Random)": render_graph_page()
else: st.warning("Config UI not implemented yet.")

# === Generation Section ===
st.divider(); st.header("4. Generate Data")
generation_possible = False; num_items_to_gen = 0
# Logic to enable button
is_instruct_and_tabular = (current_data_type == "Tabular" and st.session_state.config.get('use_case') == "Instruct Dataset")
if current_data_type == "Tabular" and st.session_state.tabular.get('columns'):
    generation_possible = True; num_items_to_gen = st.session_state.config.get('num_rows', 100)
elif current_data_type == "Excel Augmentation" and st.session_state.excel_mode.get('excel_aug_editable_columns'):
    generation_possible = True; num_items_to_gen = st.session_state.excel_mode.get('num_new_rows', 100)
elif current_data_type == "Image (Basic Shapes)":
    generation_possible = True; num_items_to_gen = st.session_state.image.get('count', 10)
elif current_data_type == "Text (Basic)":
    generation_possible = True; num_items_to_gen = st.session_state.text.get('count', 10)
elif current_data_type == "Graph (Basic Random)":
    if nx: generation_possible = True; num_items_to_gen = st.session_state.graph.get('num_nodes', 10)
    else: st.error("Cannot generate Graph: NetworkX missing.", icon="⚠️")

if generation_possible:
    button_label = f"🚀 Generate {num_items_to_gen} items" if num_items_to_gen > 0 else "🚀 Generate Data"
    if st.button(button_label, key="generate_data", type="primary", disabled=st.session_state.results['is_generating']):
        st.session_state.results['is_generating'] = True
        # Clear previous results before starting generation
        st.session_state.results['data'] = None
        st.session_state.results['message'] = None
        st.session_state.results['error'] = None
        st.rerun() # Start the generation process below
elif current_data_type in ["Tabular", "Excel Augmentation"]:
     st.warning(f"Cannot generate: Please define/upload configuration for {current_data_type}.")

# === Generation Logic Block ===
if st.session_state.results['is_generating']:
    data_type_to_generate = st.session_state.config.get('data_type')
    row_count_display = 0 # Determine item count for spinner
    if data_type_to_generate == "Tabular": row_count_display = st.session_state.config.get('num_rows', 100)
    elif data_type_to_generate == "Excel Augmentation": row_count_display = st.session_state.excel_mode.get('num_new_rows', 100)
    elif data_type_to_generate == "Image (Basic Shapes)": row_count_display = st.session_state.image.get('count', 10)
    elif data_type_to_generate == "Text (Basic)": row_count_display = st.session_state.text.get('count', 10)
    spinner_label = f"Generating {row_count_display} items..." if row_count_display > 0 else "Generating data..."
    if data_type_to_generate == "Graph (Basic Random)": spinner_label = "Generating Graph..."

    with st.spinner(spinner_label):
        # Call the appropriate action function based on data type
        if data_type_to_generate == "Tabular": generate_tabular_data_action()
        elif data_type_to_generate == "Excel Augmentation": generate_excel_data_action()
        elif data_type_to_generate == "Image (Basic Shapes)": generate_image_data_action()
        elif data_type_to_generate == "Text (Basic)": generate_text_data_action()
        elif data_type_to_generate == "Graph (Basic Random)": generate_graph_data_action()
        else:
            st.session_state.results['error'] = f"Generation action not found for type: {data_type_to_generate}"
            st.session_state.results['is_generating'] = False # Ensure flag is reset on error
    st.rerun() # Rerun to display results after generation action finishes


# === Display Results and Download Section ===
# --- MODIFICATION 2: Conditional Rendering of Results Section ---
# Only show this section if there are results (data, message, or error)
if st.session_state.results['data'] is not None or st.session_state.results['message'] or st.session_state.results['error']:
    st.divider()
    st.header("5. Results")

    # Display status message (success or error)
    if st.session_state.results['error']:
        st.error(st.session_state.results['error'], icon="🚨")
    elif st.session_state.results['message']:
        st.success(st.session_state.results['message'], icon="✅")

    results_data = st.session_state.results.get('data')

    if results_data is not None:
        current_data_type = st.session_state.config.get('data_type') # Get current type again for context

        # --- MODIFICATION 1: Add Format Selector and Adjust Download ---
        download_format = None # Variable to store chosen format

        # Check for DataFrame (Tabular standard, Excel Aug)
        if isinstance(results_data, pd.DataFrame) and current_data_type in ["Tabular", "Excel Augmentation"]:
            st.dataframe(results_data)

            # Add format selector for Tabular/Excel
            download_format = st.radio(
                "Download Format:",
                ("CSV", "JSON"),
                key="df_download_format",
                horizontal=True
            )

            try:
                data_bytes = None
                mime_type = None
                file_extension = None
                f_name_prefix = "tabular" if current_data_type == "Tabular" else "excel_augmented"

                if download_format == "CSV":
                    data_bytes = results_data.to_csv(index=False).encode('utf-8')
                    mime_type = "text/csv"
                    file_extension = "csv"
                elif download_format == "JSON":
                    # Use orient='records' for list of objects [{col:val,...}, ...]
                    json_string = results_data.to_json(orient="records", indent=4, date_format="iso")
                    data_bytes = json_string.encode('utf-8')
                    mime_type = "application/json"
                    file_extension = "json"

                if data_bytes and mime_type and file_extension:
                    st.download_button(
                        f"Download Data ({download_format})",
                        data_bytes,
                        f"synthetic_{f_name_prefix}_{datetime.now():%Y%m%d_%H%M%S}.{file_extension}",
                        mime_type,
                        key=f"dl_df_{download_format.lower()}" # Unique key per format
                    )
            except Exception as e:
                 st.error(f"Error preparing {download_format} download: {e}")

        # Check for list of strings (Text Basic OR Tabular Instruct)
        elif isinstance(results_data, list) and results_data and isinstance(results_data[0], str) and current_data_type in ["Text (Basic)", "Tabular"]:
             is_instruct_output = (current_data_type == "Tabular" and st.session_state.config.get('use_case') == "Instruct Dataset")
             preview_title = "Instruction Samples (First 10)" if is_instruct_output else "Text Samples (First 10)"
             st.subheader(preview_title)
             for i, sample in enumerate(results_data[:10]):
                 st.code(sample, language=None, line_numbers=False)

             # Add format selector for Text/Instruct
             download_format = st.radio(
                "Download Format:",
                ("TXT", "JSON"), # Changed CSV to TXT for simple text list
                key="text_download_format",
                horizontal=True
            )

             try:
                data_bytes = None
                mime_type = None
                file_extension = None
                f_name_prefix = "instruct" if is_instruct_output else "text"

                if download_format == "TXT":
                    text_content = "\n\n".join(results_data).encode('utf-8') # Double newline separator
                    data_bytes = text_content
                    mime_type = "text/plain"
                    file_extension = "txt"
                elif download_format == "JSON":
                    json_string = json.dumps(results_data, indent=4) # Dump list directly
                    data_bytes = json_string.encode('utf-8')
                    mime_type = "application/json"
                    file_extension = "json"

                if data_bytes and mime_type and file_extension:
                    st.download_button(
                        f"Download Samples ({download_format})",
                        data_bytes,
                        f"synthetic_{f_name_prefix}_{datetime.now():%Y%m%d_%H%M%S}.{file_extension}",
                        mime_type,
                        key=f"dl_txt_{download_format.lower()}" # Unique key per format
                    )
             except Exception as e:
                 st.error(f"Error preparing {download_format} download: {e}")

        # Check for Image List
        elif isinstance(results_data, list) and results_data and isinstance(results_data[0], dict) and 'image' in results_data[0] and current_data_type == "Image (Basic Shapes)":
            # NO format selector needed for Images, keep ZIP download
            st.subheader("Preview (First 10 Images)")
            cols = st.columns(5)
            for i, img_data in enumerate(results_data[:10]):
                img = img_data.get('image')
                fname = img_data.get('filename', f'img_{i}.png')
                with cols[i % 5]:
                    if img: st.image(img, caption=fname, width=100)
                    else: st.warning(f"Bad preview {i}")
            try:
                zip_buffer = io.BytesIO(); img_cnt = 0
                with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                    for i, img_data in enumerate(results_data):
                        img_bytes = image_to_bytes(img_data.get('image'))
                        fname = img_data.get('filename', f'image_{i}.png')
                        if img_bytes:
                             zf.writestr(fname, img_bytes)
                             img_cnt += 1
                if img_cnt > 0:
                     st.download_button(
                         f"Download Images ({img_cnt}) (ZIP)",
                         zip_buffer.getvalue(),
                         f"synthetic_images_{datetime.now():%Y%m%d_%H%M%S}.zip",
                         "application/zip",
                         key="dl_img_zip"
                     )
            except Exception as e:
                 st.error(f"Error creating image ZIP: {e}")

        # Check for NetworkX Graph
        elif nx and isinstance(results_data, nx.Graph) and current_data_type == "Graph (Basic Random)":
            # NO format selector needed for Graph, keep PNG/CSV downloads
            st.subheader("Graph Info")
            st.text(f"Nodes: {results_data.number_of_nodes()}, Edges: {results_data.number_of_edges()}")
            st.subheader("Visualization")
            img_buf = draw_graph(results_data)
            if img_buf:
                st.image(img_buf)
                st.download_button(
                    "Download Graph Image (PNG)",
                    img_buf.getvalue(),
                    f"synthetic_graph_{datetime.now():%Y%m%d_%H%M%S}.png",
                    "image/png",
                    key="dl_g_png"
                )
            else:
                st.warning("Graph visualization failed.")
            try:
                df_edges = nx.to_pandas_edgelist(results_data)
                csv_g = df_edges.to_csv(index=False).encode('utf-8')
                st.download_button(
                    "Download Edge List (CSV)",
                    csv_g,
                    f"synthetic_graph_edges_{datetime.now():%Y%m%d_%H%M%S}.csv",
                    "text/csv",
                    key="dl_g_csv"
                )
            except Exception as e:
                st.error(f"Error preparing graph edge list CSV: {e}")

        else:
            # Fallback if data format is unrecognized or doesn't match current mode
            if results_data is not None: # Only show if there *is* data
                 st.warning("Result format is not recognized or doesn't match expected output for the current mode.")
                 st.write("Raw results data:")
                 st.write(results_data)
    # If results_data is None, but there was a message/error, it's already displayed above.

# --- END MODIFICATION 2 ---


# --- Footer ---
# st.sidebar.markdown("---")
# st.sidebar.info("Modular Generator v_Instruct_Format")